...
=begin 
  Purpose     Make a file of tests for PWAW-U-verify
...
test_template = <<END_OF_TEMPLATE
  ...
  --assert ###expected### = PWAW-U-verify "###test_input###"
...
qt_path = IO.read(reds_libs_dir + "/path-to-red-system.txt").chomp.sub(/\/red-system\Z/, '') + 
          '/quick-test/quick-test.reds'
u_path = tests_dir.sub(/\/[tT]ests\Z/, '') + '/utf-8.reds'
test_header = test_header_template.sub(/###u_path###/, u_path).sub(/###qt_path###/, qt_path)
